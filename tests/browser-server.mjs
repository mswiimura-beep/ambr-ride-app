...
window.supabase={createClient(){
  ...
  return {auth:{getSession:async()=>({data:{session:{user:{id:'test-user',is_anonymous:true}}},error:null}),signInAnonymously:async()=>({data:{user:{id:'test-user',is_anonymous:true}},error:null}),updateUser:async()=>({data:{user:{id:'test-user'}},error:null}),signInWithOtp:async()=>({data:{},error:null}),onAuthStateChange(callback){queueMicrotask(()=>callback('INITIAL_SESSION',{user:{id:'test-user',is_anonymous:true}}));return {data:{subscription:{unsubscribe(){}}}}}},from:empty,storage:{from:()=>({createSignedUrls:async()=>({data:[],error:null}),upload:async()=>({data:{},error:null}),remove:async()=>({data:{},error:null})})},functions:{invoke:async()=>({data:null,error:{message:'disabled in browser test'}})}};
}};
</script>`;
const html = source
