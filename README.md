...
共有操作は利用者IDに結び付け、通信失敗時は所有者ID付きの送信待ちへ保存します。プロフィール画面で匿名利用者へメールを設定・確認すると、同じ利用者IDを別端末へ引き継げます。この機能には提案Migrationの適用、Supabase Authの匿名認証・Manual Linking・メールOTP設定、公開URLのRedirect URL許可が必要です。匿名認証の濫用対策としてCAPTCHAまたはTurnstileも有効化してください。
